...
- **规则调优**：收集 AI 评分记录并关联当次截图，人工标记正确分数后利用大模型自动分析偏差模式，生成优化后的评分规则，一键应用
...
8. **规则调优（可选）**
    - 每次评分完成后自动收集记录到调优列表
    - 鼠标悬停在记录行上可预览 AI 分数、正确分数、状态和当次截图
...
- **2026-05-12**：增强规则调优记录截图预览
  - 评分回调新增当次截图路径，规则调优记录自动关联 `captures/question_*.jpg`
  - 鼠标悬停在调优记录上可查看 AI 分数、人工正确分数、状态和截图缩略图
  - 清空记录或截图文件缺失时会安全关闭预览或显示提示，不影响主界面运行
